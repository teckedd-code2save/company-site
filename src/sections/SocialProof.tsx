...
function LivePill() {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-white/60">
      <span className="relative flex h-1.5 w-1.5">
        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#C084FC] opacity-75" />
        <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-[#C084FC]" />
      </span>
      Live
    </span>
  );
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-4xl font-bold tracking-tight text-white sm:text-5xl" style={{ letterSpacing: '-0.02em' }}>
            <SplitText stagger={0.03}>Built for teams that ship.</SplitText>
          </h2>
        </motion.div>

        {/* Testimonials */}
        <div className="grid gap-px md:grid-cols-3" style={{ background: 'rgba(255,255,255,0.06)' }}>
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="p-8 sm:p-10" style={{ backgroundColor: '#050505' }}
            >
              <div className="mb-6 text-3xl leading-none text-[#C084FC]/40">“</div>
              <p className="text-lg font-medium leading-relaxed text-white/90">{t.quote}</p>
              <div className="mt-8">
                <p className="text-sm font-semibold text-white">{t.name}</p>
                <p className="text-xs uppercase tracking-wider text-white/40">{t.org}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          <div className="mb-4 flex items-center gap-2">
            <LivePill />
            <span className="text-xs uppercase tracking-wider text-white/40">Package & repository signals</span>
          </div>
          ...
          <div className="grid gap-px sm:grid-cols-2 lg:grid-cols-3" style={{ background: 'rgba(255,255,255,0.06)' }}>
            {cards.map((card, idx) => (
              <a
                ...
                className="group relative p-6 transition-colors" style={{ backgroundColor: '#050505' }}
              >
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-[#C084FC]">{card.eyebrow}</span>
                  <ExternalLink className="h-3.5 w-3.5 text-white/20 transition-colors group-hover:text-white/60" />
                </div>
                <p className="text-lg font-bold tracking-tight text-white">{card.headline}</p>
                <p className="mt-2 text-sm leading-6 text-white/50">{card.detail}</p>
                <span className="mt-4 inline-block text-xs font-medium text-white/30 transition-colors group-hover:text-white/70">
                  {card.cta} →
                </span>
              </a>
            ))}
